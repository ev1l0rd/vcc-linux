...
-- Check if video is a 3D ones
local is_3d = System.doesFileExist("romfs:/three.txt")
if is_3d then
	Screen.enable3D()
end
...
while true do
	...
	drawVideo(video,UPSCREEN,is_3d)
	...
	-- Restart video
	if (checkInput(pad, X) and not checkInput(oldpad, X)) then
		JPGV.pause(video)
		JPGV.unload(video)
		video = JPGV.load("romfs:/video.jpgv")
		JPGV.start(video, NO_LOOP)
	end
	...
	if not cinema then
		...
		printText(2, 20, "X = Restart video", white, DOWNSCREEN)
		printText(2, 35, "Y = Enable/Disable Cinema", white, DOWNSCREEN)
		printText(2, 50, "START = Return Home Menu", white, DOWNSCREEN)
